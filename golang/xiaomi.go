...
// https://www.52pojie.cn/thread-1667653-1-1.html
type User struct {
	phoneNumber string
	password    string
	access      string
...
func init() {
	...
	if _, err := os.Stat("UserData.ini"); os.IsNotExist(err) {
		f, _ := os.Create("UserData.ini")
		defer f.Close()
	}
}
