...
#https://github.com/Dicer-Zz/ZhihuPicCrawler/blob/main/Spider.py
class Zhihu():
    """
        ...
        for qID in self.qIDs:
            ...
            while True:
                ...
                response = requests.get(
                    answer_url, params=data, headers=self.headers,verify=False)
                resp = response.json()
                # print(resp)
    ...
    def saveImg(self, imgUrl):
        ...
        if self.imgCounter > self.numPics:  # Maybe wrong due to concurrent.
            return
        r = requests.get(imgUrl, headers=self.headers,verify=False)
