# !/usr/bin/env python
# encoding=utf-8
# author: 李冲，改编自知乎@路人甲，附路人甲代码https://github.com/ppy2790/weixin
...
DEBUG = False
#https://github.com/lichong0525/FriendFamily/blob/master/friendsfamily.py
...
def showQRImage():
    ...
    f.close()
    time.sleep(1)
    subprocess.check_call([QRImagePath],shell=True)
    # if sys.platform.find('drawwin') >= 0:
        # subprocess.check_call([QRImagePath])
    # else:
        # subprocess.check_call(['xdg-open', QRImagePath])
...
def merger_IMG(dir_path,MemberCount):
    x = math.ceil(4*math.sqrt(MemberCount/12))
    y = math.ceil(3*math.sqrt(MemberCount/12))
    if x * 50 <= 2000:
        size = 50
    else:
        size = 2000 // x
    #计算图片个数选择合适的长宽比
    #图像个数横坐标是x个，纵坐标是y个
    toImage = Image.new('RGBA', (size*x, size*(y-1)))
    for index,file in enumerate(os.listdir(dir_path)):
        if os.path.splitext(file)[1] == ".jpg":
        #http://res.wx.qq.com/a/wx_fed/webwx/res/static/img/2KriyDK.png
            try:
                fromImage = Image.open(dir_path + "/" + file)
            except(IOError):
                url_data = requests.get("http://res.wx.qq.com/a/wx_fed/webwx/res/static/img/2KriyDK.png").content
                fromImage = open(dir_path + "/" + file,"wb")
                fromImage.write(url_data)
                fromImage.close()
                fromImage = Image.open(dir_path + "/" + file)
            fromImage =fromImage.resize((size, size), Image.ANTIALIAS)
            toImage.paste(fromImage, ((index%x) * size, (index//x) * size))
    toImage.save(dir_path + '.png')
...
def main():
    ...
    d = {}
    imageIndex = 0
    if os.path.isdir(os.getcwd()+"/portrait") == False:
        os.mkdir('portrait')
    else:
        print("警告：图片存储路径已存在")
    for Member in MemberList:
        imageIndex = imageIndex + 1
        name = os.getcwd() + '/portrait/' + str(imageIndex) + '.jpg'
        name = name.replace("\\","/")
    ...
    merger_IMG(os.getcwd() + '/portrait',MemberCount)
