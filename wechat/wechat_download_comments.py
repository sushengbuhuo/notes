...
pass_ticket = 'O9ADymTORmfcWMC+SzJBRubLWSk9ItGWSTNwcW2KmLYryfF2QFF8R/9qo5iA6YJ+T9VQwqzykeeam1nnbjkr0A=='
biz = 'MjM5NjAxOTU4MA=='
uin = ''
key = ''
...
url = 'https://mp.weixin.qq.com/s/2My565YRAztNJbqd_4ID8w'
...
"""
    ...
    if str_comment and str_msg and str_token:
        ...
        if ret == 0 or status == 'ok':
            ...
            with open(date+'-'+title+'-留言数据.csv', 'a+', encoding='utf-8-sig') as f:
                f.write('评论昵称'+','+'评论头像' +','+'评论内容' + ','+'评论时间'+ ','+'评论点赞数'+ ','+'评论回复'+ ','+'评论国家'+ ','+'评论省份'+ '\n')
            for comment in elected_comment:
                ...
                comments_excel.append([nick_name,logo_url,comment_content,comment_time,like_num,reply_content,country_name,province_name])
            ...
            with open(date+'-'+title+'-留言数据.csv', 'a+', encoding='utf-8-sig', newline='') as csvfile:
        ...
        try:
           ...
           print(err)
comments(content,date,headers,url_comment,biz,uin,key,pass_ticket,url,title)