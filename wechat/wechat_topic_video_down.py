...
for i,j,k,g in zip(msgids,links,titles,itemidxs):
	...
	try:
		title = re.search(r'<meta property="og:title" content="(.*)" />', content).group(1)
		ct = re.search(r'd\.ct = xml \? getXmlValue\(\'ori_create_time.DATA\'\) : \'(.*)\';', content).group(1)
		...
		with open('html/'+mp_name+'_'+str(randint(1,10))+'.html', 'w', encoding='utf-8') as f:
