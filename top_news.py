...
with open(f'{fname}.txt', 'a+', encoding='utf-8') as file:
    file.write('### 新闻早报\n\n')
with open(f'{fname2}.txt', 'w') as file:
    file.truncate(0)
with open(f'{fname3}.txt', 'w') as file:
    file.truncate(0)
with open(f'{fname2}.txt', 'a+', encoding='utf-8') as file:
    file.write('### 历史上的今天\n\n')
...
def zxki():
	...
	for item in response['data']['news']:
		num+=1
		with open(f'{fname}.txt', 'a+', encoding='utf-8') as f:
			f.write(item[:-1]+ '\n\n')
			# f.write(str(num)+'、'+item[:-1]+ '\n\n')
	with open(f'{fname3}.txt', 'a+', encoding='utf-8') as file:
	    file.write('### 微博热搜\n\n')
	...
	for item in response['data']:
		num+=1
		with open(f'{fname2}.txt', 'a+', encoding='utf-8') as f:
			f.write(str(num)+'、'+item['year']+'年'+item['title']+ '\n\n'+remove_html_tags(item['desc'])+ '。。。\n\n')
zxki()
# with open(f'{fname}.txt', 'a+', encoding='utf-8') as file:
